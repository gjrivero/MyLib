...
{$SCOPEDENUMS OFF}
  ...
  public
    class function New(const AURL: string): TSocketClient;
...
procedure TSocketClient.SetHeader(const key:string; const value:string);
begin
...
function TSocketClient.ClearBit(const AValue: Cardinal; const AByte: Byte): Cardinal;
begin
...
constructor TSocketClient.Create(const AURL: string);
begin
...
function TSocketClient.EncodeFrame(const AMessage: RawByteString; const AOperationCode: TOperationCode): TIdBytes;
var
  LFin, LMask: Cardinal;
...
function TSocketClient.GenerateWebSocketKey: string;
var
  LBytes: TIdBytes;
  I: Integer;
begin
...
function TSocketClient.GetBit(const AValue: Cardinal; const AByte: Byte): Boolean;
begin
  Result := (AValue and (1 shl AByte)) <> 0;
end;

procedure TSocketClient.HandleException(const AException: Exception);
var
...
function TSocketClient.IsValidHeaders(const AHeaders: TStrings): Boolean;
begin
  Result := False;
...
function TSocketClient.IsValidWebSocket: Boolean;
var
  LSpool: string;
  LByte: Byte;
...
begin
  Result := TSocketClient.Create(AURL);
end;

procedure TSocketClient.ReadFromWebSocket;
var
  LOperationCode: Byte;
  LSpool: TIdBytes;
...
procedure TSocketClient.Send(const AMessage: RawByteString);
begin
  try
    FInternalLock.Enter;
...
procedure TSocketClient.Send(const AJSONObject: TJSONObject; const AOwns: Boolean);
begin
  try
...
procedure TSocketClient.SendCloseHandshake;
begin
...
procedure TSocketClient.SetSecWebSocketAcceptExpectedResponse(const AValue: string);
var
  LHash: TIdHashSHA1;
begin
...
procedure TSocketClient.SetSubProtocol(const AValue: string);
begin
  FSubProtocol := AValue;
end;

procedure TSocketClient.StartHeartBeat;
var
...
function TSocketClient.SetBit(const AValue: Cardinal; const AByte: Byte): Cardinal;
begin
